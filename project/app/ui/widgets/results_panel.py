"""
ResultsPanel — the big right-hand pane that shows model output:
  - Headline classification + confidence + severity band
  - Per-class probability bars
  - Measurement readouts (predicted vs provided)
  - Insole configuration meter
  - Notes / warnings
"""
...
class ResultsPanel(QFrame):
    ...
    def _build(self) -> None:
        ...
        self.confidence_label.setStyleSheet(f"color: {P.text_secondary}; font-size: 13px;")
        ...
        for cls in ("Severe Flat Arch", "Flat Arch", "Normal Foot", "High Arch", "Severe High Arch"):
            row = QHBoxLayout()
            row.setSpacing(10)
            name = QLabel(cls)
            ...
            value.setStyleSheet(f"color: {P.text_primary}; font-variant-numeric: tabular-nums;")
            row.addWidget(name)
            row.addWidget(bar, 1)
            row.addWidget(value)
            probs_l.addLayout(row)
            self.prob_rows[cls] = (name, bar, value)
        body_l.addWidget(probs_box)

        # --- Measurements readout ---
        meas_box = QGroupBox("Measurements (used by model)")
        ...
        for k, lbl, unit in meas_keys:
            ...
            v.setStyleSheet(
                f"color: {P.text_primary}; font-variant-numeric: tabular-nums; font-size: 13px;"
            )
        ...
        in_l.setSpacing(6)
        self.insole_rows: dict[str, tuple[QLabel, QProgressBar]] = {}
        insole_keys = [
    ...
    # --------------------------------------------------------- public API
    def clear(self) -> None:
        self.headline.setText("Awaiting input")
        self.severity_chip.setText("—")
        self.severity_chip.setStyleSheet(self._chip_qss(P.text_muted))
        self.confidence_label.setText("Confidence —")
        for _, bar, value in self.prob_rows.values():
            bar.setValue(0)
            value.setText("0.0%")
        for v in self.meas_labels.values():
            v.setText("—")
        for _, bar in self.insole_rows.values():
            bar.setValue(0)
        self.notes_label.setText("")

    def set_result(self, result: dict) -> None:
        """Populate from a ClassificationOut-like dict."""
        ...
        for name, (_, bar, value) in self.prob_rows.items():
            p = float(probs.get(name, 0.0))
            bar.setValue(int(p * 100))
            value.setText(f"{p * 100:0.1f}%")
            # Tint the bar for the winning class.
            chunk_color = color if name == cls else P.accent_muted
            bar.setStyleSheet(
        ...
        meas = result.get("measurements_predicted") or {}
        for key, lbl in self.meas_labels.items():
            v = meas.get(key)
            lbl.setText(f"{v:.2f} {units[key]}" if v is not None else f"— {units[key]}")

        insole = result.get("insole_configuration") or {}
        for key, (_, bar) in self.insole_rows.items():
            v = float(insole.get(key, 0.0))
